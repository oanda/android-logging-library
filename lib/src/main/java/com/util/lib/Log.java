...
public class Log {
    ...
    private static void writeToFile(int priority, String tag, String msg, Throwable tr) {
        try {
            ...
            if (mContext != null && mInitialized) {
                ...
                StringBuilder currentEntry = buildEntry(priority, tag, msg, tr);
                ...
                bufferedWriter.append(currentEntry).append('\n');
                bufferedWriter.close();

                String line = currentEntry.toString();
     ...
     * @param msg The message you would like logged.
     * @param tr An exception to log.
     * @return A StringBuilder representing the entry.
     */
    private static StringBuilder buildEntry(int priority, String tag, String msg, Throwable tr) {
        // Create a StringBuilder and append our information to it, if applicable
        StringBuilder sb = new StringBuilder();

        long now = System.currentTimeMillis();
        SimpleDateFormat sdf = new SimpleDateFormat("MM-dd kk:mm:ss.SSS");

        // Append each piece of information
        sb.append(sdf.format(new Date(now)));
